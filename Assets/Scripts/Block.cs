...
public class Block : MonoBehaviour, IPointerClickHandler, IMovable
{
    ...
    public void Move()
    {
        ...
        {
            bool isHit = Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), 0.8f, _layer);
        ...
        if (Physics.Raycast(ray, out hit, 0.8f))
        {
            ...
            if(hit.transform.TryGetComponent(out ShellReceiver turret))
            {
                if (turret.IsBusy)
                {
                    ...
                    {
                        turret.GetShells(_layerForReceived, _renderer.material, 5);
                        _moveDisposible.Clear();
